...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") return json(response, 200, { ok: true, graphConfigured: Boolean(process.env.GRAPH_API_KEY), modelConfigured: Boolean(process.env.OPENAI_API_KEY || (process.env.MODEL_API_KEY && process.env.MODEL_API_URL)) });
