...
const queryPromise = fs.readFile(new URL("../../spike/graph/standardized-lending-query.graphql", import.meta.url), "utf8");
...
export async function fetchProtocolObservation(config, { apiKey, fetchImpl = fetch, gateway = "https://gateway.thegraph.com/api", capturedAt = new Date().toISOString(), timeoutMs = 12000, retries = 1 } = {}) {
  ...
  const response = await requestGraph({ fetchImpl, endpoint: `${gateway}/deployments/id/${config.deploymentId}`, apiKey, timeoutMs, retries, body: { query, variables: { protocolSlug: config.slug, tokenId: USDC, dailyFrom: Math.floor((new Date(capturedAt).getTime() - 30 * 86400000) / 1000) } } });
